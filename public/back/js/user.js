...
//  1.一进入页面渲染页面
//  如果后台返回的响应头中,设置了content-type为application/json
//  jQuery会自动按照json 格式进行解析响应结果 我们就可以不用配置dataType了
...
//  2.点击启用禁用按钮,显示模态框,通过事件委托做
  $('.lt_content tbody').on("click", ".btn", function () {
    ...
    //  用户id,以data-开头的自定义属性,可直接data("id")就可以获取
    ...
    //  之前注册的还存在,导致代码重复执行了
    //  通过off()可以将之前重复注册的事件移除,再进行事件绑定,可以保证只有一个事件绑定了
    $("#submitBtn").off().click(function () {
      
      //  发送请求 需要知道 用户id和需要设置的状态
      $.ajax({
        type: "post",
        url: "/user/updateUser",
        data: {
